...
#@st.experimental_singleton
def load_embeddings(input_dir, _model, device, model_type):
    ...
    if st.session_state.get('attribute_embeddings') == None:
        ...
        with torch.no_grad():
            if model_type == "bert":
                ...
                attribute_embeddings = _model.roberta(input_ids=argument_inputs['input_ids'],
                                                   attention_mask=argument_inputs['attention_mask'],
                                                   token_type_ids=argument_inputs['token_type_ids'])[1]
    ...
    if st.session_state.get('concept_embeddings') == None:
        ...
        with torch.no_grad():
            if model_type == "bert":
                ...
                concept_embeddings = _model.roberta(input_ids=argument_inputs['input_ids'],
                                                 attention_mask=argument_inputs['attention_mask'],
                                                 token_type_ids=argument_inputs['token_type_ids'])[1]
    ...
    if st.session_state.get('relation_embeddings') == None:
        ...
        with torch.no_grad():
            if model_type == "bert":
                ...
                relation_embeddings = _model.roberta(input_ids=argument_inputs['input_ids'],
                                                        attention_mask=argument_inputs['attention_mask'],
                                                        token_type_ids=argument_inputs['token_type_ids'])[1]
...
def main():
    ...
    if run_query and query:
        #st.session_state.results = {}
        load_embeddings(input_dir, model, device, model_type)
        #reset_results()
        #st.session_state.question = query
        st.session_state.random_question_requested = False
